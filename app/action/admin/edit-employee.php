                            ...
                            <ul class="nav flex-column">
                                <li class="nav-item">
                                    <a href="#" class="nav-link">
                                        Owned Items <span class="float-right badge bg-primary">
                                            <?php
                    ...
                    <div class="col-md-12">
                        <div class="card card-outline card-red">
                            ...
                            <thead class=" thead-dark">
                                ...
                                $result = mysqli_query($con,"SELECT issuance.issued_id, issuance.issuance_code, issuance.issued_date, 
                                items.item_name, employee.firstname, employee.lastname, office.office_name, category.category_name, 
                                con.con_desc, item_status.status_desc 
                                FROM issuance INNER JOIN items ON issuance.issued_item=items.item_id 
                                LEFT JOIN employee ON issuance.issued_to=employee.employee_id 
                                INNER JOIN office ON employee.office_id=office.office_id 
                                INNER JOIN con ON items.con_id=con.con_id INNER JOIN category ON items.category_id=category.category_id  
                                INNER JOIN item_status ON items.status_id=item_status.status_id  
                                WHERE issued_to ='".$view['employee_id']."' ORDER BY `issuance`.`issued_id` DESC;");
                                $count=1;
                                $rowCount = mysqli_num_rows($result);
                                    if($rowCount > 0){
                                        while($row = mysqli_fetch_assoc($result)){
